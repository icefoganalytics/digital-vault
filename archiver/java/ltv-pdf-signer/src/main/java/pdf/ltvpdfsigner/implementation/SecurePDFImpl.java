package pdf.ltvpdfsigner.implementation;
...
import pdf.ltvpdfsigner.custom.CustomTSAClient;
import pdf.ltvpdfsigner.intraface.SecurePDFIntf;
...
public class SecurePDFImpl implements SecurePDFIntf {
	...
	public SecurePDFImpl(String keystorePath, String keystorePassword, String tsaClient, String keyAlias, int numOfCertificates) {
		this.keystorePath = keystorePath;
		this.keystorePassword = keystorePassword;
		this.tsaClient = tsaClient;
    this.keyAlias = keyAlias;
    this.numOfCertificates = numOfCertificates;
    }

	public void signPDF(InputStream is, OutputStream os) throws IOException, KeyStoreException,
			CertificateException, NoSuchAlgorithmException, UnrecoverableKeyException {
		LOG.debug("Inside signPDF...");
		LOG.debug("", this.keystorePath);
		LOG.debug("", this.keystorePassword);
		LOG.debug("", this.tsaClient);
		LOG.debug("", this.keyAlias);
		LOG.debug("", this.numOfCertificates);
		Security.addProvider(new BouncyCastleProvider());
    sign(is, os, SIGNATURE_FIELD, PdfSigner.CryptoStandard.CADES, 0, true, PDF_PASS_OWNER.getBytes());
	}
